...
Run the notebook from top to bottom. It uses Snowpark pandas for interactive exploration, with a small number of native Workspace SQL cells where direct object inspection is useful, to:
...
Open `notebooks/02_credit_default_model_experiment.ipynb` in the same private Git-backed Workspace and use Container Runtime CPU 2.9. Do not install packages and do not run this demonstration in Snowhouse.

Run the notebook section by section rather than using **Run all**. It is structured as an interactive investigation:

- Revisit Phase 2's feature evidence and explain the profile/behaviour grouping.
- Register or retrieve the account entity and versioned external Feature Views.
- Inspect finalised-label timing and source scenario periods before choosing development, validation, and held-out boundaries, without exposing held-out target rates.
...
- Refit once, inspect held-out evidence, record a conclusion, and log the final experimental model.
